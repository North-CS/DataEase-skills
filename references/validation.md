...
- 149 Python unit/regression tests, including every v2.10.25 common-chart adapter rendering with native type/render/category conversion and authoritative field binding, indicator-native DTO defaults, China administrative-map defaults, per-view chart-data failure rejection, bounded rich-profile chart selection, interaction selector alias normalization, explicit and inferred native query cascades, background-aware query contrast, eleven built-in themes, custom/brand theme validation, SVG asset palette extraction, WCAG contrast correction, SVG preview generation, DataV indicator/table palette propagation, bounded transient-empty-preview retry, text-only visual quality scoring, compact/standard/rich deterministic complexity profiles with role diversity, principle-derived design grammar selection, semantic-v2 dense-dashboard height planning, adaptive capture resolution, dashboard/DataV custom-resolution layout, dark-theme canvas preservation, native query-component DTO construction and published-control visibility rejection, safe same-dataset linkage planning, DataV grid-only/pixel layout and aggregation preservation, component move/field/theme patching and identity allowlists, SQL/union/sync-policy model validation, secret-free L3 sync plans, portable ID mapping, plugin package hashing/multipart transport, solution metric/quality gates and permission readback/rollback, multi-dataset planning, organization-context isolation, strict filling form/task DTO normalization and task lifecycle verification, custom user-password rejection, role/resource permission normalization, administrator-role detection, dynamic user-edit risk and runtime diagnostics.
