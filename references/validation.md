...
- DataEase source tags were compared for advanced adapter routing: 2.7 legacy visual detail, 2.8+ request-style detail, 2.10.10+ linkage resource-table path, plugin availability from 2.8 and native dataset export from 2.10. The v2.10.25→v2.10.26 API diff retained the Skill's dataset, datasource, visualization, linkage, filling, role, report, webhook, SSO and plugin endpoint families; the changed `/dekey` encryption IV is adapted with 2.10.26-first and legacy fallback decryption.
- DataEase `2.10.26` focused compatibility lifecycle: an AI-generated 6×5 XLSX was uploaded through `/datasource/uploadFile` and saved as an Excel datasource; its returned physical table and field DTOs created the first dataset through the current UI-shaped `union`/`allFields` payload; a compact six-chart DataV was published, read back and passed 6/6 real chart-data requests. All temporary datasource, dataset and DataV resources were deleted afterward. Browser capture remains a runtime-dependent regression: it needs local Playwright/Chromium, while API create/readback/chart-data checks do not.
