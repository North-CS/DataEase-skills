...
class LayoutTests(unittest.TestCase):
    ...
    def test_deploy_writes_native_server_side_jump(self):
        engine = object.__new__(MultiDataEaseChartEngine)
        engine.base_url = "http://example"
        engine.client = _FakeVisualClient()
        engine.rand_id = _sequential_ids()
        engine.extract_chart_payload = _fake_extract_chart_payload
        engine.get_dataset_ctx = lambda *_args: {"XAXIS_FIELD_METADATA": {"id": "11"}}
        engine._apply_component_theme = lambda *_args: None
        engine._detect_check_version = lambda: "2.10.25"
        engine.deploy_multi(
            "跳转测试", [{"type": "bar", "title": "区域销售", "dataset_name": "1"}],
            interactions={"jumps": [{"chart": "区域销售", "field": "区域", "url": "https://example.invalid/detail", "open_mode": "_self"}]},
            publish=False, append_timestamp=False,
        )
        jump_payload = next(payload for path, payload in engine.client.posts if path == "/linkJump/updateJumpSet")
        self.assertEqual(jump_payload["linkJumpInfoArray"][0]["content"], "https://example.invalid/detail")
        self.assertEqual(jump_payload["linkJumpInfoArray"][0]["sourceFieldId"], "11")
    ...
    def test_deploy_rejects_jump_for_unknown_chart_title(self):
        ...
        with self.assertRaisesRegex(ValueError, "does not match a chart title"):
            engine.deploy_multi(
                "跳转测试", [{"type": "bar", "title": "区域销售", "dataset_name": "1"}],
                interactions={"jumps": [{"chart": "不存在", "field": "区域", "url": "https://example.invalid"}]},
                publish=False, append_timestamp=False,
            )
