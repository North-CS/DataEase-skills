...
from .field_binding import bind_field_metadata, normalized_field_metadata

class DataEaseChartEngine:
    def __init__(self, base_url, ak, sk, *, settings=None):
        self.client = DataEaseClient(base_url, ak, sk, settings=settings)
        self.base_url = self.client.base_url
        self.api_prefix = self.client.api_prefix

    def _detect_check_version(self):
    ...
    def resolve_dataset_id(self, name_or_id):
        ...
        resp = self.client.post("/datasetTree/tree", {"busiFlag": "dataset"})
    ...
    def get_dataset_ctx(self, dataset_name_or_id, x_names, y_names):
        ...
        resp = self.client.post(f"/datasetTree/details/{dataset_id}")
    ...
    def deploy(self, chart_type, title, dataset_id, x_names, y_names, layout=None):
        ...
        print(f"Deploying chart '{title}' (Type: {chart_type})...", file=sys.stderr)

        save_r = self.client.post("/dataVisualization/saveCanvas", payload)
